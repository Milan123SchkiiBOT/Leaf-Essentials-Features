...
import * as NicknameManager from "./api/nameDB.js"
import { Storage } from "./api/scriptDB.js"
        ...
        if (init.customCommandRegistry) {
            ...
            init.customCommandRegistry.registerCommand(
            {
                ...
                mandatoryParameters: [
                    ...
                    { name: "scriptName", type: CustomCommandParamType.String }
                ]
            },
            (origin, players, scriptName) => {
                if (!Array.isArray(players)) {
                    origin.sourceEntity?.sendMessage("§cNo valid players selected.");
                    return { status: CustomCommandStatus.Failure };
                }

                const code = scriptDb.get(scriptName);
                if (!code) {
                    origin.sourceEntity?.sendMessage(`§cScript '${scriptName}' not found.`);
                    return { status: CustomCommandStatus.Failure };
                }
                ...
                system.run(() => {
                    for (const player of players) {
                        try {
                            eval(`(function(player, world, system){ ${code} })(player, world, system)`);
                            origin.sourceEntity?.sendMessage(`§aRan script '${scriptName}' for §e${player.name}`);
                        } catch (e) {
                            origin.sourceEntity?.sendMessage(`§cScript '${scriptName}' error for ${player.name}: ${e.message}`);
                        }
                    }
                });
            ...
            init.customCommandRegistry.registerCommand(
                ...
                {
                    name: "leaf:nickname",
                    description: "Add or remove a nickname from players.",
                    permissionLevel: CommandPermissionLevel.GameDirectors,

                    mandatoryParameters: [
                        { name: "action", type: CustomCommandParamType.String },
                        { name: "players", type: CustomCommandParamType.PlayerSelector },
                        { name: "nickname", type: CustomCommandParamType.String, optional: true }
                    ]
                    ...
                    for (const player of players) {
                        ...
                        case "add":
                            ...
                            origin.sourceEntity?.sendMessage("§cNickname must be 1–20 characters long.");
                            break;
                            }
                            NicknameManager.set(player, nicknameRaw)
                            player.nameTag = nicknameRaw;
                            origin.sourceEntity?.sendMessage(`§aSet nickname of §e${player.name}§a to §e${nicknameRaw}`);
                            break;

                        case "remove":
                            player.nameTag = player.name;
                            NicknameManager.remove(player)
                            origin.sourceEntity?.sendMessage(`§cRemoved nickname from §e${player.name}`);
                            break;

                        default:
                            origin.sourceEntity?.sendMessage("§cInvalid action. Use 'add' or 'remove'.");
                            break;
                        }
                    }
                    });
